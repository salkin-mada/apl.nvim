--- UDP
--- Receive data from lang as UDP datagrams.
...
--- Evaluate a piece of lua code sent from lang
---@local
function Handlers.luaeval(codestring)
  ...
  if not ok then
    print('[apl] luaeval: ' .. result)
  end
end

--- Receive data from lang
---@local
...
local function on_receive(err, chunk)
  ...
  if chunk then
    ...
    if not ok then
      error('[apl] Could not decode json chunk: ' .. object)
    end
    local func = Handlers[object.action]
    assert(func, '[apl] Unrecognized handler')
    func(object.args)
  end
end
...
--- Utility function for the apl.eval API.
---@local
