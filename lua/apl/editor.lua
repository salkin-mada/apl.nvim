--- Connects nvim and the APL lang process.
--- Applies autocommands and forwards text to be evaluated to the lang process.
---@module apl.editor

local lang = require 'apl.lang'
local config = require 'apl.config'
local commands = require 'apl.commands'
local settings = require 'apl.settings'
...
--- Action that runs to highlight buffer content sent to lang.
...
--- Action that runs when buffer content is sent to lang.
--- The default is to send the content as a string to lang.
...
local function flash_once(start, finish, delay)
  local ns = api.nvim_create_namespace 'apl_flash'
  vim.highlight.range(0, ns, 'AplEval', start, finish, { inclusive = true })
...
local function fade_region(start, finish)
  ...
  api.nvim_win_set_option(id, 'winhl', 'Normal:' .. 'AplEval')
  local timer = uv.new_timer()
  local rate = 50
  local accum = 0
  ...
  for key, value in pairs(mappings) do
    ...
    if value[1] ~= nil then
      for _, v in ipairs(value) do
        vim.keymap.set(v.modes, key, v.fn, { buffer = true, desc = v.desc or "apl keymap" })
      end
    else
      vim.keymap.set(value.modes, key, value.fn, { buffer = true, desc = value.desc or "apl keymap" })
    end
  end
end
...
local function create_hl_group()
  ...
  if type(color) == 'string' then
    color = string.format('highlight default link AplEval %s', color)
  elseif type(color) == 'table' then
    color = string.format(
      'highlight default AplEval guifg=%s guibg=%s ctermfg=%s ctermbg=%s',
...
local function create_autocmds()
  local id = api.nvim_create_augroup('apl_editor', { clear = true })
  ...
  --     pattern = { '*.apl' },
  --     callback = require('apl.signature').ins_show,
  --   })
  -- end
  if config.editor.force_ft_apl then
    api.nvim_create_autocmd({
      ...
      group = id,
      desc = 'Set *.apl to filetype apl',
      pattern = '*.apl',
      command = 'set filetype=apl',
    })
  end
...
--- Get the current line and send it to lang.
...
--- Get the current block of code and send it to lang.
---@param cb An optional callback function.
---@param flash Highlight the selected text
function M.send_block(cb, flash)
  flash = flash == nil and true or flash
  local lstart, lend = unpack(vim.fn['apl#editor#get_block']())
...
function M.send_selection(cb, flash)
  flash = flash == nil and true or flash
  local ret = vim.fn['apl#editor#get_visual_selection']()
  local reg = vim.region
  local buf = api.nvim_get_current_buf()
  M.on_send(ret.lines, cb)
  -- debug
  -- for index, line in ipairs(ret.lines) do
  --     print(string.format('linenr: %s -> %s', index, line))
  -- end
  if flash then
