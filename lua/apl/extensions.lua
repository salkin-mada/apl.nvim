--- Extensions.
--- API used to manage and register third party apl extensions.
--- Heavily inspired by the extension model used by telescope.nvim
--- https://github.com/nvim-telescope/telescope.nvim
---@module apl.extensions
---@local
...
local function link_classes(name)
  ...
  local source_dir = path.concat(root_dir, 'apl')
  local dest_dir = path.concat(sc_ext_dir, 'apl-extensions')
...
local function load_extension(name)
  local ok, ext = pcall(require, 'apl._extensions.' .. name)
  if not ok then
    error(string.format('[apl] "%s" was not found', name))
  end
  if ext.setup then
...
--- This function is called by `aplExt` user command and probably not so useful on its own.
